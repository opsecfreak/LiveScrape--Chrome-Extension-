...
const App: React.FC = () => {
    ...
    useEffect(() => {
        // This check is necessary because `chrome` is not defined in a standard web context
        if (typeof chrome !== 'undefined' && chrome.tabs) {
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                if (tabs[0]?.id) {
                    setActiveTabId(tabs[0].id);
                    // Load initial data from storage
                    chrome.storage.local.get(['customers', `isScanning_${tabs[0].id}`], (result) => {
                        setCustomers(result.customers || []);
                        setIsScanning(result[`isScanning_${tabs[0].id}`] || false);
                    });
                }
            });

            // Listen for storage changes to update the UI in real-time
            // Fix: Use 'any' for StorageChange type as chrome object is declared as 'any'.
    ...
    const handleToggleScan = useCallback(() => {
        ...
        // Persist scanning state for the specific tab
